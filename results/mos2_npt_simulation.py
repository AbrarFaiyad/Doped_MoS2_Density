...
def build_mos2_structure(supercell_size=(10, 10), layers=8, vacuum=0):
    """
    Build a layered MoS2 structure using the provided CIF unit cell.
    
    Args:
        supercell_size (tuple): Size of supercell in x and y directions
        layers (int): Number of MoS2 layers (should be even since CIF is double-layer)
        vacuum (float): Vacuum space above and below structure in Angstroms
    
    Returns:
        Atoms: ASE Atoms object containing the MoS2 structure
    """
    print(f"Building MoS2 structure from CIF unit cell:")
    print(f"  - Supercell: {supercell_size[0]}x{supercell_size[1]}")
    print(f"  - Target layers: {layers}")
    print(f"  - Vacuum: {vacuum:.2f} Å")
    
    # Load the unit cell from CIF file
    cif_path = os.path.join(os.path.dirname(__file__), "..", "data", "Mo2S4.cif")
    if not os.path.exists(cif_path):
        cif_path = os.path.join("..", "data", "Mo2S4.cif")
    ...
    try:
        ...
        print(f"    - Unit cell atoms: {len(unit_cell)}")
        print(f"    - Composition: {unit_cell.get_chemical_formula()}")
        
        # The CIF contains a double layer (2 MoS2 layers), so for 8 layers we need 4 repetitions
        z_repeats = layers // 2
        if layers % 2 != 0:
            print(f"  WARNING: Requested {layers} layers, but CIF is double-layer. Using {z_repeats*2} layers.")
        
        print(f"  - CIF contains double layer, replicating {z_repeats} times in z-direction")
        ...
        print(f"  - Actual layers: {z_repeats * 2}")
        ...
        print(f"  Please ensure Mo2S4.cif exists in the data folder")
        raise
...
class MoS2NPTSimulation:
    ...
    def build_structure(self):
        ...
        try:
            # Build MoS2 supercell from CIF unit cell 
    ...
    def run_npt_simulation(self, steps=50000, log_interval=100, traj_interval=100):
        ...
        try:
            ...
            temperature_coupling = 20 * units.fs  # Fixed coupling strength
            pressure_coupling = 200 * units.fs    # Fixed coupling strength
